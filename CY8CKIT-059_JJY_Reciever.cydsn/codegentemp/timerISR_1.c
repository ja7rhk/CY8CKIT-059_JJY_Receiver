...
void timerISR_1_Stop(void)
{
    ...
    timerISR_1_SetVector(&IntDefaultHandler);
}
...
void timerISR_1_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)timerISR_1__INTC_NUMBER] = address;
}
...
cyisraddress timerISR_1_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)timerISR_1__INTC_NUMBER];
}
...
uint8 timerISR_1_GetState(void)
{
    ...
    return ((*timerISR_1_INTC_SET_EN & (uint32)timerISR_1__INTC_MASK) != 0u) ? 1u:0u;
}
